...
void GraphSplitter::getCubeFromVertexIfExists(Vertex* vertex, Graph &res, std::vector<std::pair<int, int> > &levels) {
    ...
    for (Vertex* v1 : neighbours) { // перебираем первые 3 вершины
        for (Vertex* v2 : neighbours) {
            for (Vertex* v3 : neighbours) {
                if (v1 != v2 && v1 != v3 && v2 != v3 && v1 != vertex && v2 != vertex && v3 != vertex) { // если все уникальные
                    auto NeighboursV1 = v1->Neighbours();
                    auto NeighboursV2 = v2->Neighbours();
                    auto NeighboursV3 = v3->Neighbours();
                    auto commonNeighboursV1V2 = getCommonNeighbours(NeighboursV1, // находим общих соседей
                                                                                 NeighboursV2);
                    ...
                    for (Vertex* v4 : commonNeighboursV1V2) { // перебираем следующие 3 вершины среди общих
                        for (Vertex* v5 : commonNeighboursV1V3) {
                            for (Vertex* v6 : commonNeighboursV2V3) {
                                if (v4 != vertex && v5 != vertex && v6 != vertex) {
                                    ...
                                    if (!commonNeighboursV4V5V6.empty()) { // получаем последнюю вершину
                                        Vertex* v7 = nullptr;
                                        ...
                                        if (!v7) continue;
                                        addNewCube({vertex, v1, v2, v3, v4, v5, v6, v7}, res, levels); // добавляем
                                    }
                                }
                            }
                        }
