    ...
    for (Vertex* v1 : neighbours) {
        for (Vertex* v2 : neighbours) {
            if (v1 != v2) {
                ...
                auto common_neighbours = getCommonNeighbours(neighbours_v1, neighbour_v2);
                if (!common_neighbours.empty()) {
                    Vertex* v3 = nullptr;
                    for (Vertex* v : common_neighbours)
                        if (v != vertex && v != v1 && v != v2) v3 = v;
                    if (!v3) continue;
                    if (v1 != v3 && v2 != v3 && v1 != vertex && v2 != vertex && v3 != vertex) {
                        int cnt = res.HasVertex(vertex->Id()) + res.HasVertex(v1->Id()) + res.HasVertex(v2->Id()) +
                                  res.HasVertex(v3->Id());
                        std::cout << vertex->Id() + 1 << ' ' << v1->Id() + 1 << ' ' << v2->Id() + 1 << ' ' << v3->Id() + 1 << ' ' << cnt << '\n';
                        std::cout << res.IsConnected(v1->Id(), v2->Id()) << ' ' << res.IsConnected(v2->Id(), v3->Id()) << ' ' << res.IsConnected(v1->Id(), v3->Id()) << '\n';
                        if (cnt == 0 || cnt == 2 || (cnt == 3 && !res.HasVertex(vertex->Id()) &&
                            ...
                            addNewGrid({vertex, v1, v2, v3}, res, levels);
                        }
                    }
                }
            }
